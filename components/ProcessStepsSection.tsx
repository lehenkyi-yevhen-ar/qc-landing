...
export function ProcessStepsSection({ variant = "cwa" }: { variant?: string }) {
    ...
    stepRefs.current.forEach((el, index) => {
      ...
      const observer = new IntersectionObserver(
        ([entry]) => {
          ...
          const max = intersecting.current.size > 0 ? Math.max(...intersecting.current) : 0
          setActiveUpTo(max)
        },
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="type-heading-eb-48" style={{ margin: 0, color: "#360092" }}>
          Our Process
        </h2>
        <div
          style={{
      ...
      {data.map((step, index) => {
        ...
        return (
          <div
            ...
            style={{
              display: "grid",
              gridTemplateColumns: "530px 60px 1fr",
              minHeight: index < data.length - 1 ? 350 : 328,
              position: "relative",
            }}
          >
            {/* Left: step card */}
